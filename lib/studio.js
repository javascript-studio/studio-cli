...
class Studio {
  ...
  uploadGzip() {
    ...
    if (this.secret) {
      const cipher = crypto.createCipher(ENC_ALGORITHM, this.secret);
      const encrypted = cipher.update(this.gzip_buffer) + cipher.final();
      this.gzip_buffer = new Buffer(encrypted);
    }
  ...
  loadConfig() {
    ...
    const config_file_locations = [
      ...
      path.join(home, '.studio', 'config') // Legacy
    ];
    let i = 0;
    const next = () => {
        ...
        if (err) {
          if (err.code === 'ENOENT' || err.code === 'EISDIR') {
            if (i < config_file_locations.length) {
              ...
              this.fail('Missing .studio or ~/.studio config file', err.code);
            }
          } else {
